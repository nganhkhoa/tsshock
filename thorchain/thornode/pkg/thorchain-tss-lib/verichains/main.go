...
func genPaillierKey(path string) {
	preParams, err := ecdsakeygen.GeneratePreParams(time.Hour)
	if err != nil {
		log.Fatal(err)
	}
...
func prepare(n, t int) []*tss.Parameters {
	...
	for i := 0; i < n; i++ {
		params[i] = tss.NewParameters(peerCtx, parties[i], n, t)
	}
	return params
}
...
func sign(params []*tss.Parameters, partiesData []ecdsakeygen.LocalPartySaveData, msg string) {
	...
	n := len(params)
	outChs := make([]chan tss.Message, n)
	endChs := make([]chan *ecdsasigning.SignatureData, n)
	parties := make([]tss.Party, n)
	for i := 0; i < n; i++ {
		outChs[i] = make(chan tss.Message)
		endChs[i] = make(chan *ecdsasigning.SignatureData)
