...
class TestEthereum:
    ...
    def __init__(self, base_url):
        ...
        for x in range(1, len(self.accounts)):
            self.fund_account(
                self.accounts[0], self.accounts[1], 1000000000000000000000, x == 1
            )
    ...
    def fund_account(self, from_address, to_address, amount, wait_for_commit):
        logging.info(f"fund_account: {to_address} {amount}")
        tx = {
    ...
    def deploy_init_contracts(self):
        ...
        usdt = self.deploy_erc20("USDT")
        usdc = self.deploy_erc20("USDC")

    def calculate_gas(self, msg):
        return self.default_gas + self.gas_per_byte * len(msg)

    def deploy_erc20(self, symbol, abi_file="data_erc20.json", bytecode_file="data_erc20.txt"):
        ...
        tx_hash = token.constructor(symbol, symbol).transact()
        receipt = self.web3.eth.waitForTransactionReceipt(tx_hash)
        logging.info(f"ERC20 Token Address: {symbol} => {receipt.contractAddress}")
